...
class AutomaDiagnostic {
  ...
  async checkPermissions() {
    ...
    const supportedPermissions = [
      'activeTab', 'storage', 'tabs', 'bookmarks', 'history',
      'geolocation', 'clipboardRead', 'clipboardWrite', 'alarms',
      'identity', 'scripting', 'webRequest', 'webRequestAuthProvider',
      'declarativeNetRequest', 'sidePanel', 'tabGroups', 'sessions',
      'topSites', 'favicon', 'fontSettings', 'gcm', 'idle',
      'management', 'nativeMessaging', 'power', 'privacy',
      'system.cpu', 'system.display', 'system.memory', 'system.storage',
      'unlimitedStorage'
    ];

    const unsupportedPermissions = mockExtension.manifest.permissions.filter(p => 
      !supportedPermissions.includes(p)
    );

    const supportedCount = mockExtension.manifest.permissions.filter(p => 
      supportedPermissions.includes(p)
    ).length;
    ...
    this.info('权限兼容性报告:', {
      ...
      supportedPermissions: mockExtension.manifest.permissions.filter(p => 
        supportedPermissions.includes(p)
      ),
