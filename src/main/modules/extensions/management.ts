...
export class ExtensionManager extends TypedEventEmitter<{
   ...
   * @param extensionPath - The path to the unpacked extension directory
  ...
  public async loadUnpackedExtension(extensionPath: string): Promise<string | null> {
    try {
      // Validate the extension directory
      const isValidDirectory = await isDirectory(extensionPath);
      if (!isValidDirectory) {
        throw new Error("Invalid extension directory");
      }

      // Check if manifest.json exists
      const hasManifestFile = await hasManifest(extensionPath);
      if (!hasManifestFile) {
      ...
      const success = await this.addInstalledExtension("unpacked", extensionId);
      if (!success) {
        throw new Error("Failed to add extension to store");
      }

      // Load the extension
      const extensionData = await this.extensionStore.get(extensionId);
      if (extensionData) {
        await this.loadExtensionWithData(extensionId, extensionData as ExtensionData);
      }
  ...
  /**
   * Generate a unique ID for unpacked extensions based on path
   * @param extensionPath - The path to the extension
  ...
  /**
   * Remove an unpacked extension
   * @param extensionId - The ID of the extension to remove
   * @returns True if successful, false otherwise
   */
  public async removeUnpackedExtension(extensionId: string): Promise<boolean> {
    try {
      const extensionData = this.getExtensionDataFromCache(extensionId);
      if (!extensionData || extensionData.type !== "unpacked") {
        return false;
      }
      ...
      console.error("Failed to remove unpacked extension:", error);
      return false;
    }
  }

  /**
   * Update an unpacked extension (reload from source)
   * @param extensionId - The ID of the extension to update
   * @param newSourcePath - The new source path (optional, if not provided will use original path)
   * @returns True if successful, false otherwise
   */
  public async updateUnpackedExtension(extensionId: string, newSourcePath?: string): Promise<boolean> {
    try {
      const extensionData = this.getExtensionDataFromCache(extensionId);
      if (!extensionData || extensionData.type !== "unpacked") {
        return false;
      ...
      if (newSourcePath) {
        await this.copyDirectory(newSourcePath, oldExtensionPath);
      }

      // Reload the extension
      const updatedExtensionData = await this.extensionStore.get(extensionId);
      if (updatedExtensionData) {
        await this.loadExtensionWithData(extensionId, updatedExtensionData as ExtensionData);
      }

      return true;
    } catch (error) {
      console.error("Failed to update unpacked extension:", error);
      return false;
    }
  }
}
