...
const extensionsAPI: FlowExtensionsAPI = {
  ...
  // Developer mode APIs
  loadUnpackedExtension: async (extensionPath: string) => {
    return ipcRenderer.invoke("extensions:load-unpacked-extension", extensionPath);
  },
  removeUnpackedExtension: async (extensionId: string) => {
    return ipcRenderer.invoke("extensions:remove-unpacked-extension", extensionId);
  },
  updateUnpackedExtension: async (extensionId: string, newSourcePath?: string) => {
    return ipcRenderer.invoke("extensions:update-unpacked-extension", extensionId, newSourcePath);
  }
};
...
// Expose electron API for file dialogs
const electronAPI = {
