...
// 模拟Electron环境
if (!global.electron) {
  global.electron = { app, session };
}

// 导入诊断工具
const { ExtensionPermissionManager } = require('./src/main/modules/extensions/permission-manager.ts');
const { ExtensionDiagnosticTool } = require('./src/main/modules/extensions/diagnostic-tool.ts');
...
class AutomaDiagnostic {
    ...
    try {
      ...
      if (manifest.permissions) {
        ...
        const unsupportedPermissions = [
          'proxy', 'debugger', 'webNavigation', 'cookies', 
          'downloads', 'contextMenus', 'notifications'
        ];
  ...
  async checkPermissions() {
    ...
    try {
      const permissionReport = ExtensionPermissionManager.generateCompatibilityReport(mockExtension);
      
      this.info('权限兼容性报告:', {
        score: permissionReport.score,
        supportedPermissions: permissionReport.supportedPermissions,
        unsupportedPermissions: permissionReport.unsupportedPermissions,
        warnings: permissionReport.warnings,
        recommendations: permissionReport.recommendations
      });

      if (permissionReport.score < 50) {
        this.error(`权限兼容性分数过低: ${permissionReport.score}/100`);
      } else if (permissionReport.score < 80) {
        this.warn(`权限兼容性分数较低: ${permissionReport.score}/100`);
      } else {
        this.info(`权限兼容性良好: ${permissionReport.score}/100`);
      }

    } catch (error) {
      this.error('权限检查失败', error);
    }
  }
  ...
  generateRecommendations() {
    ...
    // 基于日志生成建议
    const hasPermissionIssues = this.logs.some(log => 
      log.message.includes('不支持的权限') || log.message.includes('权限兼容性')
    );

    const hasServiceWorkerIssues = this.logs.some(log => 
      log.message.includes('Service Worker') || log.message.includes('background.bundle.js')
    );

    const hasPopupIssues = this.logs.some(log => 
      log.message.includes('Popup') || log.message.includes('popup.html')
    );
